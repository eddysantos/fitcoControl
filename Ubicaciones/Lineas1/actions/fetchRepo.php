...
if ( $_POST['emp'] || $_POST['ope']) {
  ...
  $emp = $data['emp'];
  $ope = $data['ope'];
  $query = "SELECT * FROM ct_linea_copy1
            INNER JOIN ct_lineaProd

WHERE  pk_linea = fk_linea AND ((fecha BETWEEN '$fini' AND '$ffin') AND (operacion = '$ope' OR nombre = '$emp'))";

}elseif ($_POST['request'] || $_POST['ffin']) {
  $system_callback = [];
  $data = $_POST;
  $fini = $data['request'];
  $ffin = $data['ffin'];
...
WHERE  pk_linea = fk_linea AND (fecha BETWEEN '$fini' AND '$ffin')";
};
  ...
  while ($row = $rslt->fetch_assoc()) {
    ...
    "<tr id='item' class='encabezado' style='letter-spacing: 0px;'>
      ...
      <td width='5%' class='p-0 pt-2 style='text-align: right!important;''>Linea :</td>
