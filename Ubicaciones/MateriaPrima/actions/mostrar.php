...
$system_callback = [];
$query = "SELECT * FROM ct_materiaPrima";
...
while ($row = $rslt->fetch_assoc()) {
  $mat_editar = $_SESSION['user']['mat_editar'];
  $admin = $_SESSION['user']['privilegiosUsuario'] == 'Administrador';
  $id = $row['pk_materia'];

  if ($admin || $mat_editar == 1) {
  ...
  $system_callback['data'] .="<tr class='row text-center m-0 borderojo'>
    ...
    <td class='col-md-3'>$row[mat_entradas]</td>
    <td class='col-md-4'>$row[mat_ent_correctas]</td>
